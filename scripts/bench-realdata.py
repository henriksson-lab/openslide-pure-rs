...
def main() -> int:
    ...
    for slide in slides:
        ...
        if not can_reference_open(slide):
            ...
            rows.append(row)
            print(f"{slide.name}: SKIP ({row['skipped']})")
            continue

        rust, rust_rss, rust_err = run_timed(
            [args.rust_bench, str(slide), str(args.region_size), str(args.regions_per_level)]
        )
