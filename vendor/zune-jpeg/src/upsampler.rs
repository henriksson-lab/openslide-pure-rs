...
pub(crate) fn upsample_horizontal_by_segments(
    input: &[i16], output: &mut [i16], segment_width: usize,
) {
    if input.len() <= segment_width {
        scalar::upsample_horizontal(input, &[], &[], &mut [], output);
        return;
    }
...
mod tests {
    ...
    #[test]
    fn horizontal_segmented_upsample_resets_each_segment_boundary() {
        let input = [10, 20, 30, 40, 50, 60];
        let mut output = [0; 12];

        upsample_horizontal_by_segments(&input, &mut output, 3);

        assert_eq!(output, [10, 13, 18, 23, 28, 30, 40, 43, 48, 53, 58, 60]);
    }

    #[test]
    fn horizontal_segmented_upsample_does_not_blend_across_mcu_boundary() {
        let input = [10, 10, 10, 10, 200, 200, 200, 200];
        let mut segmented = [0; 16];
        let mut unsegmented = [0; 16];

        upsample_horizontal_by_segments(&input, &mut segmented, 4);
        scalar::upsample_horizontal(&input, &[], &[], &mut [], &mut unsegmented);

        assert_eq!(
            segmented,
            [10, 10, 10, 10, 10, 10, 10, 10, 200, 200, 200, 200, 200, 200, 200, 200]
        );
        assert_ne!(segmented[7], unsegmented[7]);
        assert_ne!(segmented[8], unsegmented[8]);
    }

    #[test]
    fn horizontal_segmented_upsample_uses_scalar_for_single_segment_row() {
        let input = [3, 11, 19, 27];
        let mut segmented = [0; 8];
        let mut scalar = [0; 8];

        upsample_horizontal_by_segments(&input, &mut segmented, input.len());
        super::scalar::upsample_horizontal(&input, &[], &[], &mut [], &mut scalar);

        assert_eq!(segmented, scalar);
    }
