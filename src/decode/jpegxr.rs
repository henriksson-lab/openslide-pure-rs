...
/// Pure-Rust JPEG XR backend using the optional `jpegxr-pure-rs` crate.
#[cfg(feature = "jpegxr")]
#[derive(Debug, Clone, Copy, Default)]
pub struct PureRustJpegXrDecoder;

#[cfg(feature = "jpegxr")]
impl JpegXrDecoderBackend for PureRustJpegXrDecoder {
    fn name(&self) -> &'static str {
        "jpegxr-pure-rs"
    }

    fn supports_pixel_format(&self, pixel_format: JpegXrPixelFormat) -> bool {
        pure_supported_pixel_format(pixel_format)
    }

    fn decode(&self, request: JpegXrDecodeRequest<'_>) -> Result<JpegXrImage> {
        decode_with_pure_backend(request)
    }
}

#[cfg(feature = "jpegxr")]
static DEFAULT_JPEG_XR_DECODER: PureRustJpegXrDecoder = PureRustJpegXrDecoder;
...
#[cfg(feature = "jpegxr")]
fn decode_with_pure_backend(request: JpegXrDecodeRequest<'_>) -> Result<JpegXrImage> {
    let mut decoder = jpegxr_pure_rs::Decoder::from_slice(request.data)
        .map_err(|err| pure_decode_error(request.context, err))?;
    let image = decoder
        .decode()
        .map_err(|err| pure_decode_error(request.context, err))?;
    ...
    let pure_format = image.pixel_format();
    ensure_pure_format_supported(request, pure_format)?;
    let data = normalize_pure_data(image, request)?;
    Ok(JpegXrImage {
        width,
        height,
...
#[cfg(feature = "jpegxr")]
fn pure_supported_pixel_format(pixel_format: JpegXrPixelFormat) -> bool {
    matches!(
        pixel_format,
...
#[cfg(feature = "jpegxr")]
fn pure_decode_error(context: &str, err: jpegxr_pure_rs::Error) -> OpenSlideError {
    OpenSlideError::Decode(format!("{context} JPEG XR pure-Rust decode failed: {err}"))
}

#[cfg(feature = "jpegxr")]
fn ensure_pure_format_supported(
    request: JpegXrDecodeRequest<'_>,
    pure_format: jpegxr_pure_rs::PixelFormat,
) -> Result<()> {
    if pure_format_matches_request(pure_format, request.options.pixel_format) {
        return Ok(());
    }
    Err(OpenSlideError::UnsupportedFormat(format!(
        "{} JPEG XR pure-Rust decoder returned {:?}, which cannot be normalized to {:?}",
        request.context, pure_format, request.options.pixel_format
    )))
}

#[cfg(feature = "jpegxr")]
fn pure_format_matches_request(
    pure_format: jpegxr_pure_rs::PixelFormat,
    requested: JpegXrPixelFormat,
) -> bool {
    use jpegxr_pure_rs::{BitDepth, ChannelOrder, ColorFormat, SampleFormat};

    match requested {
        JpegXrPixelFormat::Gray8 => {
            pure_format.color_format() == ColorFormat::Luma
                && pure_format.bit_depth() == BitDepth::Eight
        }
        JpegXrPixelFormat::Gray16 => {
            pure_format.color_format() == ColorFormat::Luma
                && matches!(
                    pure_format.bit_depth(),
                    BitDepth::Sixteen | BitDepth::SignedSixteen
                )
        }
        JpegXrPixelFormat::GrayFloat => {
            pure_format.color_format() == ColorFormat::Luma
                && pure_format.bit_depth() == BitDepth::Float
        }
        JpegXrPixelFormat::Gray32 => {
            pure_format.color_format() == ColorFormat::Luma
                && matches!(
                    pure_format.sample_format(),
                    SampleFormat::FixedPoint | SampleFormat::UnsignedInteger
                )
                && matches!(
                    pure_format.bit_depth(),
                    BitDepth::ThirtyTwo | BitDepth::SignedThirtyTwo
                )
        }
        JpegXrPixelFormat::Bgr24 => {
            pure_format.color_format() == ColorFormat::Rgb
                && pure_format.bit_depth() == BitDepth::Eight
                && !pure_format.has_alpha()
                && matches!(
                    pure_format.channel_order(),
...
#[cfg(feature = "jpegxr")]
fn normalize_pure_data(
    image: jpegxr_pure_rs::DecodedImage,
    request: JpegXrDecodeRequest<'_>,
) -> Result<Vec<u8>> {
    use jpegxr_pure_rs::ChannelOrder;

    let width = image.width();
    let height = image.height();
    let pure_format = image.pixel_format();
    let stride = image.stride();
    let bytes_per_pixel = pure_format.bytes_per_pixel();
    let expected =
    ...
    let row_len = (width as usize)
        ...
        .ok_or_else(|| {
            OpenSlideError::Decode(format!(
                "{} JPEG XR decoded row size overflows usize",
                request.context
            ))
        })?;
    if row_len > stride {
        return Err(OpenSlideError::Decode(format!(
            "{} JPEG XR pure-Rust decoder returned stride {stride}, expected at least {row_len}",
            request.context
        )));
    }
    let mut data = Vec::with_capacity(expected);
    for row in image.pixels().chunks_exact(stride).take(height as usize) {
        data.extend_from_slice(&row[..row_len]);
    }
    if data.len() != expected {
        return Err(OpenSlideError::Decode(format!(
            "{} JPEG XR pure-Rust decoder returned {} compact bytes, expected {expected}",
            request.context,
            data.len()
        )));
    }

    match request.options.pixel_format {
        JpegXrPixelFormat::Gray8
        | JpegXrPixelFormat::Gray16
        | JpegXrPixelFormat::GrayFloat
        | JpegXrPixelFormat::Gray32 => Ok(data),
        JpegXrPixelFormat::Bgr24 | JpegXrPixelFormat::Bgr48 | JpegXrPixelFormat::BgrFloat
            if pure_format.channel_order() == ChannelOrder::Bgr =>
        {
            Ok(data)
        }
        JpegXrPixelFormat::Bgr24 | JpegXrPixelFormat::Bgr48 | JpegXrPixelFormat::BgrFloat
            if pure_format.channel_order() == ChannelOrder::Rgb =>
        {
            swap_rgb_channels(&mut data, bytes_per_pixel);
            Ok(data)
        }
        ...
        JpegXrPixelFormat::Bgra32 if pure_format.channel_order() == ChannelOrder::Rgb => {
            ...
            if pure_format.has_alpha() && pure_format.premultiplied_alpha() {
                unpremultiply_bgra(&mut data);
            }
            Ok(data)
        }
...
mod tests {
    ...
    #[cfg(feature = "jpegxr")]
    #[test]
    fn default_config_uses_pure_backend_when_feature_enabled() {
        let config = JpegXrDecoderConfig::default();
        assert_eq!(config.backend_name(), "jpegxr-pure-rs");
        assert!(config
            .backend
    ...
    #[cfg(feature = "jpegxr")]
    #[test]
    fn pure_backend_advertises_translated_czi_pixel_layouts() {
        let backend = PureRustJpegXrDecoder;
        ...
        for pixel_format in [
            ...
            assert!(
                ...
                "{pixel_format:?} should be advertised by the pure-Rust JPEG XR backend"
            );
        }
        ...
        assert!(
            !backend.supports_pixel_format(pixel_format),
            "{pixel_format:?} should stay disabled until the pure-Rust backend exposes it"
        );
    }
