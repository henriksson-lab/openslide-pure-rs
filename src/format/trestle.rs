...
fn validate_trestle(tiff: &TiffFile) -> Result<()> {
    ...
    if first.ascii(TAG_IMAGEDESCRIPTION).is_none() {
        return Err(OpenSlideError::UnsupportedFormat(
...
impl TrestleSlide {
    fn open(tiff: TiffFile) -> Result<Self> {
        ...
        let mut properties = trestle_properties(&image_description);
        add_standard_properties(&mut properties, &tiff);

        let overlaps = parse_overlaps(properties.get("trestle.OverlapsXY"));
    ...
    fn decode_tile(&self, level_index: u32, tile_no: u64) -> Result<CachedTile> {
        ...
        let tile = if level.planar_config == PLANARCONFIG_SEPARATE {
            ...
            match level.compression {
                ...
                COMPRESSION_NONE => decode_uncompressed_tile(level, &raw)?,
                COMPRESSION_PACKBITS => {
...
fn trestle_properties(description: &str) -> HashMap<String, String> {
...
fn add_standard_properties(props: &mut HashMap<String, String>, tiff: &TiffFile) {
    ...
    if let Some(value) = props.get("trestle.Background Color") {
        if let Ok(bg) = u32::from_str_radix(value.trim(), 16) {
            props.insert(
                properties::PROPERTY_BACKGROUND_COLOR.into(),
                format!("{:06x}", bg & 0x00ff_ffff),
            );
        }
    }

    if let Some(dir) = tiff.directory(0) {
        if let Some(value) = dir.float(tiff.endian, TAG_XRESOLUTION) {
            props.insert("tiff.XResolution".into(), format_float(value));
            props.insert(properties::PROPERTY_MPP_X.into(), format_float(value));
        }
        if let Some(value) = dir.float(tiff.endian, TAG_YRESOLUTION) {
            props.insert("tiff.YResolution".into(), format_float(value));
            props.insert(properties::PROPERTY_MPP_Y.into(), format_float(value));
        }
    }
}

fn add_level_properties(props: &mut HashMap<String, String>, levels: &[TrestleLevel]) {
...
fn parse_overlaps(value: Option<&String>) -> Vec<(i32, i32)> {
...
fn associated_path(path: &Path) -> Option<PathBuf> {
    for extension in ["Full", "full", "FULL", "Macro", "macro"] {
        let mut associated = path.to_path_buf();
        associated.set_extension(extension);
...
fn decode_separate_tile(path: &Path, level: &TrestleLevel, tile_no: u64) -> Result<CachedTile> {
...
fn merge_jpeg_tables(tile: &[u8], tables: Option<&[u8]>) -> Vec<u8> {
    let Some(tables) = tables else {
        return tile.to_vec();
    };
    if tables.is_empty() || !starts_with_soi(tile) || has_jpeg_tables(tile) {
        return tile.to_vec();
    }

    let table_payload = strip_soi_eoi(tables);
...
fn has_jpeg_tables(data: &[u8]) -> bool {
    let mut idx = 2;
...
mod tests {
    ...
    #[test]
    fn reads_macro_sidecar_with_case_variant_and_non_jpeg_payload() {
        ...
        assert_eq!(image.pixel(0, 0), [0xff, 0x00, 0x00, 0xff]);
        assert_eq!(image.pixel(1, 0), [0x00, 0xff, 0x00, 0xff]);
    ...
    #[test]
    fn parses_trestle_overlap_with_commas_and_semicolons() {
        assert_eq!(
            parse_overlaps(Some(&"1, 2;3 4".to_string())),
            vec![(1, 2), (3, 4)]
        );
    }

    #[test]
    fn parses_trestle_objective_power_with_x_suffix() {
        let mut props = trestle_properties("Objective Power=20X;Background Color=00ffee");
        let tiff = TiffFile {
            path: PathBuf::new(),
        ...
        add_standard_properties(&mut props, &tiff);
    ...
    fn decodes_planar_separate_ycbcr_tile() {
        ...
        let tile = decode_separate_tile(&path, &level, 0).unwrap();
    ...
    fn make_trestle_tiff_with_software(software: &str) -> Vec<u8> {
        ...
        let desc = b"Background Color=00ffee;Objective Power=20;OverlapsXY=1 0\0";
