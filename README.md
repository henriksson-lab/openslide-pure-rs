...
|------------------|------------|--------------------|------------|-------|
| Aperio | `.svs`, `.tif` | Supported | Partial, experimental | Tiled TIFF pyramid reads for raw, JPEG, deflate, PackBits, planar-separated 8-bit raw/PackBits/deflate, contiguous YCbCr, downscaled contiguous 16-bit gray/RGB with uniform singleton or per-sample BitsPerSample, omitted BitsPerSample defaulting to 8 bits for each declared sample, and LZW fallback tiles including 16-bit Gray/GrayA/RGB/RGBA whole-image fallback; normalizes common Aperio metadata variants, CR/LF/pipe-delimited ImageDescription fields, background color, and associated-image names including compact thumbnail labels; JPEG 2000 compression tags are routed through decoder API scaffolding that inspects codestream/JP2 headers, color space, tile geometry, and COD coding-style metadata before explicit no-backend errors, but pixel decoding requires a codec dependency and is not implemented |
| DICOM | `.dcm` | Supported | Partial, experimental | Reads native 8-bit, unsigned 16-bit, signed 8-bit, or signed 16-bit MONOCHROME1/MONOCHROME2 with rescale/window support including LINEAR_EXACT and SIGMOID VOI, native RGB and YBR_FULL in contiguous or planar layout, native YBR_FULL_422 including odd-width row padding, and PALETTE COLOR with case-insensitive coded-string and common separator/spacing alias handling for photometric values, Explicit VR Big Endian native frames, Deflated Explicit VR Little Endian native frames, and encapsulated JPEG Baseline WSI frames including compressed RGB without PlanarConfiguration, tolerant WSI ImageType role tuples with trailing components and spaced/hyphenated/underscored role aliases, per-frame positioned tile order including first observed optical path/z-plane selection for 2D views with auditable selected/skipped/missing-tile metadata, Basic Offset Table fragment grouping diagnostics, broader associated-image role aliases, same-series sibling discovery metadata with concatenation completeness/duplicate/missing-part diagnostics, dimension organization, concatenation, and TotalPixelMatrixOrigin metadata, acquisition/manufacturer/window/protocol/device properties, objective-power suffix normalization, and additional WSI properties; JPEG 2000 transfer syntaxes are routed through decoder API scaffolding but require a codec dependency for decoding, and multi-file pixel assembly plus user-selectable multi-plane/multi-optical-path views are not implemented |
...
(`zune-jpeg`), PNG, BMP, TIFF LZW, PackBits, deflate, and Zstd support. JPEG
2000 and JPEG XR pixel decoding are still not implemented. JPEG 2000 has a
decoder-facing API boundary with request/options/result types, source/region/tile
context, backend capabilities, a backend config wrapper, and a default
no-backend implementation; codestream/JP2 headers are inspected and stream,
output, and region capabilities are checked before requests reach that backend
hook. JPEG XR has a decoder-facing API with request/options types, expected CZI
...
implementation. Format handlers route through a central decoder facade so a
future backend can be wired without touching every caller. Actual decoding still
needs format-specific color/bit-depth handling and fixture coverage for Aperio,
DICOM, and Zeiss data. The available JPEG XR crate is a wrapper around
...
documents JPEG 2000, JPEG XR, broad multifile CZI, and multi-file/multi-plane
DICOM work here as blind translation/scaffolding unless and until real decoder
or broader file-assembly support and fixture-backed tests are added.
